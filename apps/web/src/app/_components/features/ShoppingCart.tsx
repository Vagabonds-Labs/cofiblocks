...
export default function ShoppingCart({ closeCart }: ShoppingCartProps) {
  ...
  const { data: cartItems, isLoading } = api.shoppingCart.getItems.useQuery({ cartId });
      ...
      <div className="absolute right-0 top-14 w-96 bg-white p-4 shadow-xl">
        ...
        {isLoading ? (
          ...
          <>
            <div className="mt-4 flex flex-col gap-4">
              {cartItems?.map((item: CartItem) => (
                <div key={item.id} className="flex items-center justify-between">
                  ...
                  <button onClick={() => handleRemoveItem(item.id)} type="button">Remove</button>
                </div>
              ))}
            </div>
            ...
            <button className="mt-4 w-full rounded-xl bg-primary p-4 text-white" type="button">
              Checkout
            </button>
          </>
        )}
