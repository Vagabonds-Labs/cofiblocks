...
export const marketplaceRouter = createTRPCRouter({
	...
	getProductPrice: publicProcedure
        .input(z.object({ tokenId: z.string(), tokenAmount: z.string(), paymentToken: z.enum(['ETH', 'USDC', 'USDT']) }))
        .query(async ({ input }) => {
            return await getProductPrice(BigInt(input.tokenId), BigInt(input.tokenAmount), input.paymentToken as PaymentToken);
        }),
    ...
    buyProduct: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
            ...
            return await buyProduct(
                BigInt(input.tokenId), BigInt(input.tokenAmount), input.paymentToken as PaymentToken, userAuthData
            );
        }),
    
    buyProducts: protectedProcedure
        .input(z.object({ tokenIds: z.array(z.string()), tokenAmounts: z.array(z.string()), paymentToken: z.enum(['ETH', 'USDC', 'USDT']) }))
        .mutation(async ({ ctx, input }) => {
            ...
            const userAuthData = await registerUser(ctx.session.user.email, "1234");
            let tokenIds_parsed = input.tokenIds.map(BigInt);
            let tokenAmounts_parsed = input.tokenAmounts.map(BigInt);
            if (tokenIds_parsed.length !== tokenAmounts_parsed.length) {
                throw new Error("Token IDs and token amounts must have the same length");
            }
            return await buyProducts(
                tokenIds_parsed, tokenAmounts_parsed, input.paymentToken as PaymentToken, userAuthData
            );
        }),
    ...
    createProduct: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
            ...
            const userAuthData = await registerUser(ctx.session.user.email, "1234");
            const tx = await createProduct(BigInt(input.initialStock), BigInt(input.price_usd), userAuthData);
            // TODO: get token id from tx
            return { token_id: 1 };
        }),

    claimProducer: protectedProcedure
        .mutation(async ({ ctx }) => {
            if (!ctx.session.user.email) {
                throw new Error("User email not found");
            }
    ...
    claimConsumer: protectedProcedure
        .mutation(async ({ ctx }) => {
            if (!ctx.session.user.email) {
                throw new Error("User email not found");
            }
