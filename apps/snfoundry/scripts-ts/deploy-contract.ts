...
const executeDeployCalls = async (options?: UniversalDetails) => {
	...
	try {
		const { transaction_hash } = await deployer.execute(deployCalls, options);
		...
		if (deployCalls.length > 1) {
			...
			deployCalls.splice(0, deployCalls.length, ...firstHalf);
			await executeDeployCalls(options);
			deployCalls.splice(0, deployCalls.length, ...secondHalf);
			await executeDeployCalls(options);
		}
	}
};
...
const exportDeployments = () => {
	...
	const finalDeployments = resetDeployments
		? deployments
