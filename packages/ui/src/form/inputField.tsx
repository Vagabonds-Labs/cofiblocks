...
type InputFieldProps<T extends FieldValues> = {
	...
	disabled?: boolean;
	onChange?: (value: string) => void; // Añadido: Prop onChange para detectar cambios
};
...
function InputField<T extends FieldValues>({
	...
	inputClassName,
	disabled,
	onChange, // Añadido: Recibe onChange desde el componente padre
}: InputFieldProps<T>) {
	const {
		field,
		fieldState: { error },
	...
	// Manejador del evento onChange que actualiza el estado y ejecuta la función externa
	const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
		field.onChange(e); // Mantener la sincronización con react-hook-form
		if (onChange) {
			onChange(e.target.value); // Ejecuta la función pasada como prop
		}
	};
		...
		<div className={cs("flex flex-col gap-2 font-inter", className)}>
			...
			<div className={cs("relative", inputClassName)}>
				<input
					...
					disabled={disabled}
					value={field.value || ""} // Asegúrate de que el valor siempre esté definido
					onChange={handleChange} // Usar el manejador del evento
				/>
			</div>
			{error && (
